export default class Router {
  ...
  render(path, params = {}) {
    ...
    if (
      (_this.Routes[this.errorParam] && !matchedPath) ||
      matchedPath == this.errorParam
    ) {
      document.querySelector('body').innerHTML = '';
      document
        .querySelector('body')
        .appendChild(new _this.Routes[this.errorParam]());
    } else if (matchedPath) {
  ...
  matchPath(path) {
    ...
    for (let i = 0; i < routes.length; i++) {
      ...
      for (let j = 0; j < paths.length; j++) {
        if (
          absolutePaths[j] == paths[j] ||
          (paths[j][0] == '{' &&
            paths[j][paths[j].length - 1] == '}' &&
            absolutePaths[j])
        ) {
