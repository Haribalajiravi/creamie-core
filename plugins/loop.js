export default class Loop {
    ...
    extend(property, element, items) {
        ...
        for (let index = 0; index < items.length; index++) {
            ...
            let newFragment = this.createNodeListFragment(this.cloneCopy[property]);
    ...
    insertData(fragment, obj) {
        let _this = this;
        let els = fragment.querySelectorAll(`[${_this.loopItemAttribute}]`);
        els.forEach((el) => {
    ...
    isMatched({
        ...
        return {
            getter: (element.hasAttribute(this.loopAttribute) && attribute == this.loopAttribute) ? true : false,
            setter: (element.hasAttribute(this.loopAttribute) && attribute == this.loopAttribute && Array.isArray(currentValue)) ? true : false
        }
    }
}